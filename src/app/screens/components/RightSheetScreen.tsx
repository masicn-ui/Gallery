...
import {
  RightSheet,
  type RightSheetRef,
} from '../../../shared/components/RightSheet';
...
export function RightSheetScreen() {
  const navigation = useNavigation();
  const [basic, setBasic] = useState(false);
  const [narrow, setNarrow] = useState(false);
  const [wide, setWide] = useState(false);
  const [noBackdrop, setNoBackdrop] = useState(false);
  const imperativeRef = useRef<RightSheetRef>(null);
    ...
    >
      <ShowcaseSection title="Default (75%)">
        <Text variant="caption" color="textSecondary">
          Slides in from the right at 75% screen width. Swipe right or tap
          backdrop to dismiss.
        </Text>
      ...
      <RightSheet visible={basic} onClose={() => setBasic(false)}>
        <FilterContent title="Filters" onClose={() => setBasic(false)} />
      </RightSheet>

      <RightSheet visible={narrow} onClose={() => setNarrow(false)} width={0.5}>
        <FilterContent title="Narrow (50%)" onClose={() => setNarrow(false)} />
      </RightSheet>

      <RightSheet visible={wide} onClose={() => setWide(false)} width={0.9}>
        <FilterContent title="Wide (90%)" onClose={() => setWide(false)} />
      </RightSheet>
      ...
      <RightSheet ref={imperativeRef} onClose={() => {}}>
        <FilterContent
